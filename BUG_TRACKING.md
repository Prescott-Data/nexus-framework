...
The `GetToken` function in `nexus-broker/pkg/handlers/callback.go` was not explicitly ordering the tokens by creation date. When multiple tokens existed for a single connection (as is the case after a refresh), the database's default retrieval order was not guaranteed, leading to an older token being returned.
