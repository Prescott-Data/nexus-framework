...
func (p RetryPolicy) normalized() RetryPolicy {
	...
	if q.MinDelay <= 0 {
		q.MinDelay = 200 * time.Millisecond
	}
	if q.MaxDelay <= 0 {
		q.MaxDelay = 2 * time.Second
	}
	if q.MaxDelay < q.MinDelay {
		q.MaxDelay = q.MinDelay
	}
	return q
}
...
func readGatewayError(r io.Reader, status int) error {
	...
	if len(b) > 0 {
		return fmt.Errorf("gateway error %d: %s", status, strings.TrimSpace(string(b)))
	}
...
func (c *Client) do(ctx context.Context, method, urlStr string, headers map[string]string, body []byte) (*http.Response, error) {
	...
	for i := 0; i <= pol.Retries; i++ {
		if c.Logger != nil {
			c.Logger.Infof("http %s %s attempt %d", method, urlStr, i+1)
		}
		resp, err = attempt()
		if err == nil && resp != nil {
			return resp, nil
		...
		if c.Logger != nil {
			c.Logger.Infof("retrying in %s: %v", delay, err)
		}
		select {
		case <-ctx.Done():
			return nil, ctx.Err()
